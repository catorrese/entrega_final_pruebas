...
async function recursiveExploration(page, link, depth, parentState){
  ...
  if(isLogin == 0) {
    await page.type('input[name="identification"]', config.values.emailInput)
    await page.type('input[name="password"]', config.values.passwordInput)
    await page.click('button[id="ember12"]')
    await new Promise(r => setTimeout(r, 7000));
    await page.click('a[href="#/staff/"]');
    await page.click('a[href="#/staff/brahian/"]')
